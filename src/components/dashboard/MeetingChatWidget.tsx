...
export const MeetingChatWidget = () => {
  ...
  } = useChatSessions({ contextType: "dashboard" });
  ...
  return (
    <div className="rounded-xl p-4 bg-muted">
        ...
        <ScrollArea className="flex-1 pr-2" ref={scrollRef}>
          {messages.length === 0 ? (
            <div className="text-center text-muted-foreground text-sm py-8">
              ...
              <p className="text-xs mt-2 opacity-70">z.B. "Was waren die wichtigsten Action Items?"</p>
            </div>
          ) : (
      ...
      <form onSubmit={handleSubmit} className="flex gap-2">
        <Input value={input} onChange={e => setInput(e.target.value)} placeholder="Frag etwas über deine Meetings..." disabled={isStreaming} className="flex-1 bg-background/70" />
