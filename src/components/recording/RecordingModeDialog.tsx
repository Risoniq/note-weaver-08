...
import { Monitor, AppWindow, Globe, Camera, AlertTriangle } from 'lucide-react';
...
export function RecordingModeDialog({ children }: Props) {
  ...
  const handleConfirm = () => {
    if (pendingMode) {
      startRecording(pendingMode, includeWebcam);
    }
  ...
  return (
    <>
      ...
      <AlertDialog open={showConsent} onOpenChange={(open) => { if (!open) handleCancel(); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-sm leading-relaxed">
              Sie sind gesetzlich verpflichtet, alle Gesprächsteilnehmer vor Beginn der Aufnahme
              darüber zu informieren, dass das Gespräch aufgezeichnet wird.
              <br /><br />
              Eine Aufnahme ohne Zustimmung aller Beteiligten ist nicht rechtmäßig.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={handleCancel}>Abbrechen</AlertDialogCancel>
            <AlertDialogAction onClick={handleConfirm}>Verstanden &amp; Starten</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </>
